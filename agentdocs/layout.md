...
- `src/prompts/prompt.rs` owns Werk's single-pass template substitution and AQL expressions. `json_path.rs` parses and evaluates JSON paths over selected tasks, results, or events. `prompts/mod.rs` supplies runtime string values and directives. Agent and Task pass prompts to Werk.
