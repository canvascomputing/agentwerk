...
- `crates/agentwerk-py/examples/pit_stop/` is a uv project using the local Python binding and an example-local Vite/Three.js frontend. Its shared `layout.json` supplies car, staging and equipment geometry; `movement.py` plans explicit crew journeys; `sim_clock.py` pauses shared physical time while agents decide. Seeded setup and authoritative mechanical state live in `simulation.py`; crew park beside their seeded station. Version 6 records destination facing, jack grip geometry and clearance, and visible parking groups; replay retains earlier formats. The frontend separates recorded motion, persistent equipment, hand articulation, and titles set through `werk.on_event`, and milestone-derived timers; it bundles its pixel font with its license under `public/licenses/`.
- Keep agent composition, mechanical state, transport, playback, and rendering separate. Physical tools enforce mechanical constraints. Agentwerk Conditions start each task from physical milestones or earlier reports, and the Chief reviews once and owns GO/HOLD. The browser observes events.
