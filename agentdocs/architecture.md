...
- The per-ticket transcript is the one shape that does not fit either trait. `Comments` (in `agents::tickets`) is a free type with `append(dir, key, &Comment)` and `load(dir, key) -> Vec<Comment>`. It writes one JSON line per `Comment` to `tickets/<key>/comments.jsonl`; `load` reconstructs the in-memory transcript by picking the newest `(ticket.<ts>.json, comments.<ts>.jsonl)` compaction-pair (paired check: `ticket.<ts>.json` is the commit marker) as the base and merging running-log entries with strictly greater `created_at` on top. The pattern is per-key, so the single-fixed-filename `Append` trait does not generalize cleanly; promote to a trait only when a second per-key transcript appears.
- One agent processes one ticket at a time (claim is atomic), so `add_comment` and the compaction-pair write for one key are sequential within a single loop task. No per-key lock is needed for either path.
- Crate-internal helpers `write_atomic` (tmp+rename) and `append_line` (`O_APPEND` + newline) are the only places that touch the filesystem. They are `pub(crate)` so trait impls colocated with their types can call them; by convention nothing outside a `Persist` or `Append` impl reaches for them. Two documented exceptions: `TicketSystem::write_tool_output` writes single-shot flat files that don't fit either trait; the compaction-pair write inside `agents::loop::compaction::compact` calls `write_atomic` twice (comments file first, then the header file as commit marker) because the pair is a two-file atomic-ish operation that no single in-memory value owns.
