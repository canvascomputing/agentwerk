...
use agentwerk::{Agent, AgentOutput, Batch, Event, EventKind, Tool, ToolResult};
...
fn build_worker(
    ...
    Agent::new()
        ...
        .provider(provider)
        .model_name(model)
        .identity_prompt(WORKER_PROMPT)
        .instruction_prompt(format!("Compute S = sum_{{k={lo}}}^{{{hi}}} k^2."))
        .tool(python_tool())
...
fn failure_reason(output: &AgentOutput) -> String {
